...
import { useCachedData } from '@/hooks/useLocalStorage'
import { CACHE_KEYS, CACHE_TTL } from '@/lib/localStorage'
...
export default function OptimizedProfileWithCache() {
  ...
  const { 
    ...
    error, 
    refetch 
  } = useCachedData<UserData>(
    `${CACHE_KEYS.USER_DATA}_${session?.user?.id || 'guest'}`,
    async () => {
      if (!session?.user?.id) {
  ...
  }, [status, router, userData, loading, refetch])
  ...
  if (showLoading && loading) {
    return <FastLoadingScreen text="Loading profile..." />
  }
